...
export const meetingsRouter = createTRPCRouter({
    ...
    .query(async ({input, ctx}) => {
        ...
        .then((agents) =>
            agents.map((agent) => ({
                ...agent,
                image: generateAvatarUri({seed: agent.id, variant: "botttsNeutral"}),
            }))
        );
    ...
    .query(async ({input, ctx}) => {
        ...
        return existingMeeting.summary;
    }),
    ...
    .query(async ({ctx, input }) => {
        ...
        const {page, pageSize, search, agentId, status} = input.pagination;
